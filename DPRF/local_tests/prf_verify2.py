...
if __name__ == "__main__":
    ...
    # PRF computation 
    w = sum([alpha[i]* hashval[i] for i in range(u)]) % q
    z = int(int(w) * pqratio) % p
    ...
    #big_value = big_value % tau 
    frack = (Decimal(int(big_value)) - Decimal(int(w)))/Decimal(int(q)) # for correction to be applied by client 

    print("frack:", frack)
    print("frack/q:", int(frack)/(int(q)))
    #print("frack mod tau:", int(frack) % tau)
