...
def dprf_mode(nid, DPRF_PORT):
    ...
    s.bind (( '',DPRF_PORT))
    s.listen (1) #TODO: Make it just one 
    ...
    while 1:
        ...
        print("\n\n\n")
        print("time to answer:", (r_end - r_start))
        print("process time to answer:", (rp_end - rp_start))
...
def node_thread(nid):
    ...
    dprf_thread = threading.Thread(target=dprf_mode, args=(nid,DPRF_PORT ))
    dprf_thread.daemon = False
    dprf_thread.start()
    dprf_thread.join()

    dprf_end  = time.process_time()
    dprf_time = (dprf_end - dprf_start)*1000
    print("DPRF time:", dprf_time)
