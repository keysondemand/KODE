...
#from DPRF.dprf import partial_eval
from DPRF.dprfzkp import partial_eval
...
def handleBrocastComplaints(M,nid):
    ...
    for pid in peers.keys():
        try :
            ...
            DPRINT("broadcastedComplaint:", broadcastedComplaint)
        except:
            print("Exception in handling broadcast complaint")
            continue
            ...
            if (broadcastedComplaint['msg_type'] == "COMPLAINT_INDICATION"):
                ...
                if (am_I_accused == "yes"):
                    ...
                    #for pid in list(connections.keys()):
                    #    print("In handle broadcast compalint -- pid:", pid)
...
def complaintVerify(complaints, M, nid):
    ...
    for accstn in accusations:
        ...
        complaint_count[accused_nid] += 1 
        if complaint_count[accused_nid] > (N_NODES // 3):
...
def node_thread(nid):
    ...
    if MALICIOUS:
        timingfilename = "./tmp/dkgtiming_malicious_n_"+str(N_NODES)+".csv"
    else:
        timingfilename = "./tmp/dkgtiming_n_"+str(N_NODES)+".csv"
    ...
    print("PHASEX: Finished the DKG process")
    '''
    
    print("\nNow entering DPRF mode ...")

    dprf_start = time.process_time()
